...
func (e *SolIndexer) IndexTransfer(ctx context.Context, inBlockNum chan uint64, outTransfers chan []*eth.TokenTransfer) {
	for {
		select {
		case num := <-inBlockNum:
			cli, err := eth.Client(e.Endpoints)
...
func (e *SolIndexer) IndexToken(ctx context.Context, inTransfers chan []*eth.TokenTransfer, outTransfer chan *eth.TokenTransfer) {
	for {
		select {
...
func (e *SolIndexer) IndexContract(ctx context.Context, inTransfer chan *eth.TokenTransfer, findContractCreator bool) {
	for {
		select {
